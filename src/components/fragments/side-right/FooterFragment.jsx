...
const FooterFragment = () => {
  ...
  return (
    <Element
      className="py-16 border-t border-[var(--bg-light)]"
      name="footer"
      id="footer"
    >
      <motion.footer
      ...
      >
        <div className="text-[var(--text-secondary)] text-sm md:text-base leading-relaxed max-w-2xl mx-auto">
          {descriptionWords.map((word, index) => (
            <motion.span
              key={index}
              className={`${
                isBold(word) ? "text-[var(--primary)] font-semibold" : ""
              }`}
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
        ...
        >
          © {new Date().getFullYear()} Muhammad Ketsar Ali Abi Wahid - All rights
          reserved.
        </motion.p>
      </motion.footer>
    </Element>
